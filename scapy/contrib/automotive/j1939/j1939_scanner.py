    ...
    for da in scan_range:
        ...
        try:
            for _sa in src_addrs:
                ...
                _sa_resps = []  # type: List[CAN]

                def _rx(pkt, _da=_da, _sa=_sa):
                    # type: (CAN, int, int) -> None
                    if not (pkt.flags & _CAN_EXTENDED_FLAG):
                        return
                    _, pf, ps, sa = _j1939_decode_can_id(pkt.identifier)
                    if sa == _da and pf == J1939_PF_ADDRESS_CLAIMED and (
                        (ps == _sa or ps == J1939_GLOBAL_ADDRESS) and sa != ps
                    ):
                        log_j1939.debug(
                ...
                def _send_probe(_da=_da, _sa=_sa):
                    # type: (int, int) -> None
                ...
                rx_sock.sniff(
                    ...
                    stop_filter=lambda _: bool(_sa_resps),
                )
    ...
    if not skip_functional:
        ...
        try:
            for _sa in src_addrs:
                ...
                def _rx_functional(pkt, _sa=_sa):
                    # type: (CAN, int) -> None
                    if not (pkt.flags & _CAN_EXTENDED_FLAG):
                        return
                    if stop_event is not None and stop_event.is_set():
                    ...
                    if not force and noise_ids is not None and sa in noise_ids:
                        return
                    if pf == diag_pgn | 0x01 and ps == _sa:
                        data = bytes(pkt.data)
            ...
            for _sa in src_addrs:
                ...
                def _rx(pkt, _da=_da, _sa=_sa):
                    ...
                    if not (pkt.flags & _CAN_EXTENDED_FLAG):
                        return
                    _, pf, ps, sa = _j1939_decode_can_id(pkt.identifier)
                    if sa == _da and pf == diag_pgn and ps == _sa and sa != ps:
                        data = bytes(pkt.data)
                        if data[:3] in _UDS_TESTER_PRESENT_RESPS:
                            log_j1939.debug(
                ...
                for req in reqs:
                    ...
                    def _send_probe(_da=_da, _sa=_sa, _req=req):
                        # type: (int, int, bytes) -> None
                        _pre_probe_flush(rx_sock)
                        can_id_a = _j1939_can_id(_SCAN_PRIORITY, diag_pgn, _da, _sa)
                        send_sock.send(CAN(identifier=can_id_a, flags="extended", data=_req))
                        log_j1939.debug(
